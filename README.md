...
- CORET 使用 GTC 提供的 4 MHz 时基，并通过 NuttX `arch_alarm` 驱动
  CLIC IRQ 7。系统时钟不再依赖 idle 任务，因此 DHCP、DNS、LVGL 和
  LPWORK 同时运行时，`sleep`、`usleep` 与超时仍能持续推进。
